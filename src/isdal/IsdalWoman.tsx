...
const Scene: React.FC<{idx:number;dur:number;img:string;cap:string;sub:string}> = ({idx,dur,img,cap,sub}) => {
  ...
  return <AbsoluteFill style={{background:'#11100e',overflow:'hidden'}}>
    <Img src={staticFile(`isdal/${img}`)} style={{width:'100%',height:'100%',objectFit:'cover',transform:`translateX(${x}px) scale(${zoom})`,filter:'sepia(.28) saturate(.72) contrast(1.15) brightness(.75)'}}/>
    <AbsoluteFill style={{background:'radial-gradient(ellipse at 50% 43%, transparent 28%, rgba(0,0,0,.78) 100%)'}}/>
    <AbsoluteFill style={{opacity:.16,background:'repeating-linear-gradient(90deg,rgba(255,255,255,.14) 0 1px,transparent 1px 5px)'}}/>
    ...
    {idx===2 && <div style={{position:'absolute',right:70,top:430,width:530,height:670,border:'5px solid #e0b64b',background:'rgba(18,12,8,.7)',transform:`translateY(${(1-spring({frame:frame-70,fps,config:{damping:12}}))*500}px) rotate(-5deg)`,boxShadow:'15px 20px 0 rgba(0,0,0,.6)'}}/>}
